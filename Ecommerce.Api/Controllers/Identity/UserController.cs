    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> Edit(string id, [FromBody] UserCreateDto model)
        {
            ...
            if (user != null)
            {
                ...
                var assignRole = await _identityService.AddOrUpdateUserRole(user.Id, model.UserRoles);
                if (result == true)
                {
                   var roleAssignResult= await _identityService.AddOrUpdateUserRole(data.Id, model.UserRoles);
                }

                if (result==true)
                {
                    return CreatedAtRoute("getById", new { id = data.Id }, _mapper.Map<UserReturnDto>(data));
                    //return Ok(result);
                }
            }
        ...
        public async Task<IActionResult> GetAllUsers([FromQuery] UserCriteriaDto criteriaDto)
        {
            ...
            if (users != null)
            {
                ...
                {
                    user.Image = Path.Combine("UserImage", user.Image ?? "no-image.png");
