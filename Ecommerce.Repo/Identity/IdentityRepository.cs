...
using Ecommerce.Models.ReturnDto;
using Ecommerce.Models.ReturnDto.Permissions;
    ...
    public class IdentityRepository
    {
        ...
        public async Task<(Result Result, int UserId)> Register(UserForRegisterDto userForRegister)
        {
            ...
            var user = new User
            {
                ...
                //PhoneNumber = userForRegister.Mobile,
        ...
        public async Task<(Result Result, int UserId)> CreateUser(UserCreateDto userForRegister)
        {
            ...
            var userCodePartSequence = (long)1;
            var lastUser = await _context.Users.OrderByDescending(c => c.Id).FirstOrDefaultAsync();
            if (lastUser == null)
            {
                userCodePartSequence = 1;
            }
            else
            {
                userCodePartSequence = lastUser.UserCodePartSequence + 1;
            }
            
            var UserCode = DateTime.Now.Year + DateTime.Now.Month.ToString("d2") + userCodePartSequence;
            ...
            var user = new User
            {
                ...
                UserCodePartSequence = userCodePartSequence,
                UserCode = UserCode,
                CreatedOn = DateTime.Now,
            ...
            if (result.Succeeded)
            {
                ...
                var assignRole = await AddOrUpdateUserRole(user.Id, userForRegister.UserRoles);
            }
        ...
        public async Task<User> GetUserById(string userId)
        {
            ...
            if (result != null)
            {
                var role = await _userRoleRepo.GetRolesByUserId(result.Id);
                result.UserRoles = role;
            }
            return result;
        }
